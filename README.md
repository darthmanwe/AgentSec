...
not. The numbers below are generated from a committed evaluation artifact by
`uv run task report`, and CI fails if the README drifts from it. The Axis-B headline
reproduces in about a minute with no credentials and no network (`uv run task eval`); Axis A
took one funded run, $1.35 of a $18 ceiling.
